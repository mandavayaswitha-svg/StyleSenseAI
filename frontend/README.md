...
Get Whole Outfit Styled (Mode 1): Curates a complete outfit from scratch based on user gender, occasion, age, budget, and skin tone.
Build Around My Item (Mode 2): Recommends complementary pieces (e.g., matching pants, shoes, watches) to pair with a base clothing item that the user already owns.
Style Chat: A natural, human-like chat interface for instant fashion, trends, color coordination, and styling advice.
...
Frontend (React): Handles webcam capture, image uploads, profile preferences, dynamic grids, and chat history.
Backend (Flask): Processes images via OpenCV, handles user authentication, computes budget shares, and coordinates AI prompting.
Database (MongoDB): Stores persistent user profiles, past preference history, and saved styling recommendations.
Mermaid diagram
...
Step 1 (Upload): User uploads a face photo. OpenCV clusters skin pixels, detects the skin tone, and maps it to a color palette.
Step 2 (Preferences): User selects Gender, Occasion, Age, and Total Budget. A Style Inspiration panel is shown on the right.
Step 3 (Results): Backend query prompts Groq LLM using the skin tone color palette. Frontend displays 3 outfit options in a full-width grid.
Mode 2: Build Around My Item
Step 1 (Upload): User uploads a photo of their clothing item (or enters description manually).
Step 2 (Preferences): User selects matching clothing categories (e.g. Pants, Shoes, Watch), enters total budget, and selects/detects skin tone via a modal prompt. Style Inspiration mood board displays on the right.
Step 3 (Results): Backend OpenCV extracts the base item color/pattern, allocates budgets, prompts Groq, and returns 3 coordinated matching looks in a full-width grid.
